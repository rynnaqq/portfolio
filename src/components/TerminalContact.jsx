...
export default function TerminalContact({ terminalRefFromParent }) {
  // Contact Form State
  ...
  const handleTerminalSubmit = (e) => {
    ...
    switch (cmd) {
      ...
      case 'spatial':
        newHistory.push({
          type: 'output',
          text: 'GLOBAL SPATIAL 3D ENGINE:\n  - Renderer: WebGL2 Hardware Accelerated (Three.js)\n  - Active Shaders: Glossy Clearcoat Core + Neon Wireframe Cage\n  - Particle Volume: 1,200 Interactive Starfield Nodes\n  - Physics: Scroll & Mouse Interpolation with Gyroscopic Rings\n  - Status: 120 FPS render target locked',
        });
        break;
      case 'about':
        newHistory.push({
      ...
      case 'hire':
        newHistory.push({
          type: 'output',
          text: 'CONTRACT INTAKE:\nPlease use the form to your left or dispatch an encrypted transmission to kaelen.vance.dev@gmail.com.',
        });
        break;
      case 'whoami':
        newHistory.push({
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 pb-6 border-b border-white/10">
          <div>
            ...
            <h2 className="font-display text-4xl sm:text-5xl md:text-6xl font-black uppercase tracking-tight text-white">
              LET'S BUILD <br />
              <span className="gradient-headline">THE FUTURE</span>
            </h2>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-6 space-y-6">
            <Card3D
              maxRotation={4}
              className="p-8 rounded-3xl bg-surface-card/75 backdrop-blur-xl border border-white/10 relative overflow-hidden shadow-glow-card"
            >
              <div className="absolute top-0 right-0 w-64 h-64 bg-neon-lime/5 blur-3xl pointer-events-none" />

              <div className="flex items-center justify-between mb-6 pb-4 border-b border-white/5">
                <div>
                  <h3 className="font-display text-xl font-bold text-white">Direct Inquiry</h3>
                  <p className="text-xs font-mono text-zinc-400 mt-1">Average response window: &lt; 12 hours</p>
                </div>
                <span className="px-2.5 py-1 rounded-full bg-neon-lime/10 border border-neon-lime/30 font-mono text-[11px] text-neon-lime">
                  ENCRYPTED
                </span>
              </div>
              ...
              {isSubmitted ? (
                ...
                >
                  <div className="w-16 h-16 rounded-full bg-neon-lime/20 border border-neon-lime flex items-center justify-center mx-auto text-neon-lime shadow-neon-lime">
                    <Check className="w-8 h-8" />
                  </div>
                  <h4 className="font-display text-2xl font-bold text-white">TRANSMISSION RECEIVED</h4>
                ...
                <form onSubmit={handleFormSubmit} className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 rounded-xl bg-oled/80 border border-white/10 text-white font-sans text-sm focus:outline-none focus:border-neon-lime focus:ring-1 focus:ring-neon-lime transition-colors"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-4 py-3 rounded-xl bg-oled/80 border border-white/10 text-white font-mono text-sm focus:outline-none focus:border-neon-lime focus:ring-1 focus:ring-neon-lime transition-colors"
                    >
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full px-4 py-3 rounded-xl bg-oled/80 border border-white/10 text-white font-sans text-sm focus:outline-none focus:border-neon-lime focus:ring-1 focus:ring-neon-lime transition-colors resize-none"
                    />
                  </div>
                  ...
                  <Magnetic strength={0.2} className="w-full">
                    <button
                      type="submit"
                      className="w-full py-4 rounded-xl bg-neon-lime text-black font-mono font-bold text-sm tracking-wider hover:shadow-neon-lime transition-all flex items-center justify-center gap-2"
                    >
            ...
            <div className="p-6 rounded-2xl bg-surface-card/75 backdrop-blur-xl border border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-surface-200/80 border border-white/10 flex items-center justify-center text-neon-lime">
                  <Mail className="w-5 h-5" />
                </div>
                <div>
                  <div className="font-mono text-xs text-zinc-500 uppercase">DIRECT INBOX</div>
              ...
              <button
                ...
                className="px-4 py-2 rounded-lg bg-surface-200 border border-white/10 hover:border-neon-lime font-mono text-xs text-zinc-300 hover:text-white flex items-center gap-2 transition-all self-start sm:self-auto"
              >
                {copiedEmail ? (
                  <>
          ...
          <div className="lg:col-span-6 space-y-6">
            ...
            <Card3D
              maxRotation={4}
              className="rounded-3xl bg-zinc-950/85 backdrop-blur-xl border border-white/15 overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.8)]"
            >
              {/* Terminal Window Chrome */}
              <div className="px-4 py-3 bg-surface-card/90 border-b border-white/10 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 font-mono text-xs text-zinc-400">
                  <TerminalIcon className="w-3.5 h-3.5 text-neon-lime" />
                  <span>bash — 80x24 (spatial link online)</span>
                </div>
              ...
              <div
                ...
                className="p-5 h-[340px] overflow-y-auto font-mono text-xs space-y-2.5 text-zinc-300 select-text"
              >
              ...
              <div className="px-4 py-2 bg-surface-card/60 border-t border-white/5 flex flex-wrap items-center gap-2 font-mono text-[11px] text-zinc-500">
                <span>QUICK RUN:</span>
                {['help', '3d', 'about', 'skills', 'matrix', 'clear'].map((cmd) => (
                  <button
                    key={cmd}
                    onClick={() => {
                    ...
                    className="px-2 py-0.5 rounded bg-surface-200 border border-white/5 hover:border-neon-lime text-zinc-400 hover:text-neon-lime transition-all"
                  >
                    {cmd}
                  </button>
                ))}
            ...
            {/* Social Channels & Availability Matrix */}
            <div className="p-6 rounded-3xl bg-surface-card/75 backdrop-blur-xl border border-white/10 space-y-4">
                  ...
                  return (
                    <Magnetic key={social.name} strength={0.25}>
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="group p-4 rounded-xl bg-surface-200/80 border border-white/5 hover:border-neon-lime flex flex-col justify-between transition-all duration-300"
                      >
                        <div className="flex items-center justify-between mb-3 text-zinc-400 group-hover:text-neon-lime transition-colors">
                          <Icon className="w-5 h-5" />
                          <ArrowUpRight className="w-3.5 h-3.5 opacity-40 group-hover:opacity-100 transition-opacity" />
                        </div>
                        <div>
                          <div className="font-mono text-xs font-bold text-white group-hover:text-neon-lime transition-colors">
                            {social.name}
                          </div>
                          <div className="font-mono text-[10px] text-zinc-500">{social.handle}</div>
                        </div>
                      </a>
                    </Magnetic>
                  );
