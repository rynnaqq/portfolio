...
export default function About() {
  ...
  // Measure real live browser frame rate for realistic tech HUD
  useEffect(() => {
    let frameCount = 0;
    ...
    <section id="about" className="py-24 sm:py-32 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-8 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 pb-6 border-b border-white/10">
          <div>
            ...
            <h2 className="font-display text-4xl sm:text-5xl md:text-6xl font-black uppercase tracking-tight text-white">
              ENGINEERED FOR <br />
              <span className="gradient-cyber">PURE IMPACT</span>
            </h2>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          {/* Card 1: Core Narrative Bio (Spans 7 cols) with 3D Perspective */}
          <Card3D
            maxRotation={5}
            className="md:col-span-7 p-8 sm:p-10 rounded-3xl bg-surface-card/75 backdrop-blur-xl border border-white/10 flex flex-col justify-between group hover:border-white/20 transition-all shadow-glow-card"
          >
            <div className="absolute top-0 right-0 w-80 h-80 bg-neon-purple/15 blur-3xl pointer-events-none" />
            
            <div>
              <span className="font-mono text-xs text-zinc-500 uppercase tracking-wider block mb-4">
                // ARCHITECTURAL DOSSIER
              </span>

              <h3 className="font-display text-2xl sm:text-3xl font-extrabold text-white mb-6 leading-snug">
                "We treat code as architectural steel, animations as physical inertia, and latency as technical failure."
              </h3>

              <div className="space-y-4 text-zinc-400 font-normal leading-relaxed text-sm sm:text-base">
                <p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-8 pt-8 border-t border-white/10">
              <div className="flex items-start gap-3">
                <CheckCircle2 className="w-4 h-4 text-neon-lime shrink-0 mt-1" />
                <div>
                  <h4 className="text-xs font-mono font-bold text-white uppercase">Zero Jank</h4>
                  <p className="text-xs text-zinc-400 mt-0.5">Strict 60-120fps render budget on all viewports.</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <CheckCircle2 className="w-4 h-4 text-neon-lime shrink-0 mt-1" />
                <div>
                  ...
                  <p className="text-xs text-zinc-400 mt-0.5">Optimized edge compute & instant data sync.</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <CheckCircle2 className="w-4 h-4 text-neon-lime shrink-0 mt-1" />
                <div>
                  ...
                  <p className="text-xs text-zinc-400 mt-0.5">Award-winning dark luxury art direction.</p>
                </div>
              </div>
            </div>
          </Card3D>

          {/* Card 2: Interactive 3D Model Inspector + Telemetry HUD (Spans 5 cols) */}
          <Card3D
            maxRotation={5}
            className="md:col-span-5 p-8 rounded-3xl bg-surface-card/75 backdrop-blur-xl border border-white/10 flex flex-col justify-between relative shadow-glow-card"
          >
            ...
            <div>
              ...
              {/* Live Telemetry Data */}
              <div className="p-3.5 rounded-xl bg-oled/70 border border-white/5 space-y-2 font-mono text-xs">
          ...
          {/* Cards 3 - 6: Metrics & Stat Counters with 3D Card Hover */}
          {portfolioData.stats.map((stat, index) => (
            <div key={index} className="md:col-span-3">
              <Card3D
                maxRotation={8}
                className="h-full p-6 sm:p-7 rounded-2xl bg-surface-card/75 backdrop-blur-xl border border-white/10 hover:border-neon-lime/40 transition-all group shadow-glow-card"
              >
                <span className="font-mono text-xs text-zinc-500 block mb-2">// METRIC_0{index + 1}</span>
                <div className="font-display text-4xl sm:text-5xl font-black text-white group-hover:text-neon-lime transition-colors">
                  {stat.value}
                </div>
                <div className="font-mono text-xs font-semibold text-zinc-200 mt-2 uppercase tracking-wide">
                  {stat.label}
                </div>
                <p className="text-xs text-zinc-500 mt-1 leading-relaxed">{stat.detail}</p>
              </Card3D>
            </div>
          ))}
        </div>
